...
def preprocess_song(song_input, mdx_model_params, song_id, is_webui, input_type, progress=None):
    ...
    main_vocals_dereverb_path = run_mdx(mdx_model_params, song_output_dir, os.path.join(mdxnet_models_dir, 'Reverb_HQ_By_FoxJoy.onnx'), main_vocals_path, invert_suffix='DeReverb', exclude_main=True, denoise=True)
...
def no_audio_effects(song_input, voice_model, pitch_change, keep_files,
    ...
    try:
        ...
        if urlparse(song_input).scheme == 'https':
            ...
            if os.path.exists(song_input):
                ...
                raise Exception(f'{song_input} does not exist.')
        ...
        # isolate voice
        vocals_path, instrumentals_path = run_mdx(mdx_model_params, song_dir, os.path.join(mdxnet_models_dir, 'UVR-MDX-NET-Voc_FT.onnx'), orig_song_path, denoise=True, keep_orig=True)

        # AI reference
        ai_vocals_path = os.path.join(song_dir, f'{os.path.splitext(os.path.basename(orig_song_path))[0]}_{voice_model}_p{pitch_change}.{output_format}')
        voice_change(voice_model, vocals_path, ai_vocals_path, pitch_change, 'rmvpe', 0.5, 3, 0.25, 0.33, 128, False)

        return ai_vocals_path

    except Exception as e:
        raise Exception(f"Error in simple voice conversion: {str(e)}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-apply-effects', action=argparse.BooleanOptionalAction, help='Whether to apply audio effects and mixing (default: True)')
    parser.add_argument('--simple-conversion', action=argparse.BooleanOptionalAction, help='Only perform voice separation and AI inference, without mixing')
